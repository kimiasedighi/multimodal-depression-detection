...
class TSFFM(nn.Module):
    def __init__(self, num_classes=3, pretrained=False, fle_dim=256):
        super().__init__()
    ...
    def forward(self, body_img, face_img, labels=None, class_weights=None):
        ...
        ce = lambda x: F.cross_entropy(x, labels, weight=class_weights)

        # loss = ce(logits_f) + ce(logits_b) + ce(logits_fused) + ce(logits_final)
        loss = ce(logits_final)
        return loss, out
